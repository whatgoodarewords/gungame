...
**Non-goals (v1):** accounts/progression/ranks, anti-cheat beyond server authority + validation, mobile/touch, spectator, demos/replays, voice, text chat (killfeed is the channel), room browser, projectiles (hitscan + melee only), matchmaking beyond quickplay, map workshop, multi-style rendering as a shipped feature, room host controls (config immutable at creation), horizontal scale-out (v1 = one container on the dream server).
...
- Weapon art: 6 models, CC0 bases restyled to the committed RenderStyle. Positional audio.
...
- Rooms: quickplay = fullest non-full room of either mode; none → create **default: Gun Game, standard gravity**. **Room creation (the path to every non-default config):** the name-entry screen has two actions — **Play** (quickplay) and **Create room** (mode picker: Scoutzknivez / Gun Game; gravity variant for Gun Game) which creates the room and shows its shareable invite link. The `create` request rides the join hello; the picker UI ships in Phase 4's menu work, and Phases 2–3 create non-default rooms via URL query params (dev path, documented). Invite links reference the room by id (config lives server-side, immutable); full → "quickplay instead?". **Map follows mode** (the vertical scoutz map / the gun-game arena — 2 maps, 2 modes; rotation is post-v1). Empty rooms die after 5 min. No DB.
...
- **Gun Game:** FFA, pistol → SMG → shotgun → rifle → scout → knife; knife-kill demotes victim one level; first knife-level kill wins. Standard gravity default; scoutz-gravity variant at creation.
...
- Weapons all hitscan (knife 1.6 m cone); damage/ROF in `shared/weapons.ts` (Phase 3 issue); scout = 1-shot torso/head, slow ROF, zoom-accuracy model.
...
4. **Solo-content trap.** 2 maps, 6 weapons, CC0 bases, one style, greybox until Phase 4. (Content is Fable-owned; code throughput is the Codex lane.)
